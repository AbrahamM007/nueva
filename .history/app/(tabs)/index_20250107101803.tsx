...
const App: React.FC = () => {
  ...
  const renderWatchScreen = () => (  <FlatList  key="watch"  ListHeaderComponent={  <>  <Text style={styles.watchHeader}>Watch</Text>  {renderVideoCard()}  </>  }  data={[]}  renderItem={null}  contentContainerStyle={[styles.homeScreenScroll, { alignItems: "center" }]}  />  );  const renderWatchScreen = () => (  <FlatList  key="watch"  ListHeaderComponent={  <>  <Text style={styles.watchHeader}>Watch</Text>  {renderVideoCard()}  <Text style={[styles.sectionHeader, { marginTop: 16 }]}>Past Videos</Text>  </>  }  data={pastVideos}  keyExtractor={(item) => item.id.videoId}  renderItem={({ item }) => (  <TouchableOpacity  style={{ flex: 1, margin: 5 }}  onPress={() => {  // When a past video is tapped, play it in the video player  setLatestVideoId(item.id.videoId);  }}  >  <Image  source={{ uri: item.snippet.thumbnails.medium.url }}  style={{ width: "100%", aspectRatio: 16 / 9, borderRadius: 10 }}  />  </TouchableOpacity>  )}  numColumns={2}  contentContainerStyle={styles.homeScreenScroll}  />  );
