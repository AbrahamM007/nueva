...
// New interface for Service planning
    ...
    <View style={[styles.container, { padding: 16 }]}>
      ...
      <ScrollView style={{ marginTop: 16 }}>
        {loadingBible ? (
          ...
          <Text style={{ fontSize: 16, color: "#2D6A4F" }}>{bibleText}</Text>
        )}
      </ScrollView>
    </View>
  );
...
const App: React.FC = () => {
  ...
  // 2.6. Profile-Image Upload 
  // 2.7. Modals for Creating Ministries / Events / Services
  ...
  const handleCreateService = () => {
    ...
    const newService: ServiceItem = {
      ...
      songs: serviceSongs.split(",").map(s => s.trim()).filter(Boolean),
      roles: {},
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={[styles.appButton, styles.addButton]} onPress={item.onPress}>
            ...
            <Text style={styles.appsButtonText}>{item.label}</Text>
          </TouchableOpacity>
        )}
      ...
      <View style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            let creatorName = "Unknown";
            const userCreator = users.find((u) => u.id === item.createdBy);
            if (userCreator) creatorName = userCreator.name;
            return (
              <View style={styles.largeCard}>
                ...
                {item.people.length > 0 && (
                  <Text style={styles.cardSubText}>
                    People:{" "}
                    {item.people.map((pid) => users.find((u) => u.id === pid)?.name || "Unknown").join(", ")}
                  </Text>
                )}
              </View>
            );
  ...
  const renderServicesPage = () => {
    ...
    return (
      <View style={styles.container}>
        ...
        >
          <Ionicons name="add-circle-outline" size={24} color="#2D6A4F" style={{ marginRight: 8 }} />
          <Text style={styles.appButtonText}>Plan Service</Text>
        </TouchableOpacity>
        <FlatList
          data={sortedServices}
          keyExtractor={(item) => item.id}
          renderItem={({ item }) => (
            <View style={styles.largeCard}>
              ...
              {/* Extend to show roles and multimedia as needed */}
            </View>
          )}
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalCard}>
              ...
              <ScrollView style={{ maxHeight: 400 }}>
                ...
                />
                {/* Extend for roles and multimedia upload as desired */}
              </ScrollView>
              <View style={styles.modalButtonRow}>
                <TouchableOpacity
                  style={styles.modalCancelButton}
              ...
              .map((m) => (
                <View key={m.id} style={styles.largeCard}>
                  ...
                  {m.people.length > 0 && (
                    <Text style={styles.cardSubText}>
                      People:{" "}
                      {m.people.map((pid) => users.find((u) => u.id === pid)?.name || "Unknown").join(", ")}
                    </Text>
                  )}
                </View>
              ))
              ...
              .map((ev) => (
                <View key={ev.id} style={styles.largeCard}>
                  ...
                  {ev.attendees.length > 0 && (
                    <Text style={styles.cardSubText}>
                      Attendees:{" "}
                      {ev.attendees.map((uid) => users.find((u) => u.id === uid)?.name || "Unknown").join(", ")}
                    </Text>
                  )}
                </View>
              ))
