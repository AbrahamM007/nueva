  ...
  const pickProfileImage = async () => {
    try {
      ...
      const result = await ImagePicker.launchImageLibraryAsync({
        ...
        aspect: [1, 1],              // Optional: enforce aspect ratio (square)
        quality: 0.5,                // Optional: set image quality
      });
  
      if (!result.cancelled && result.uri && loggedInUser) {
        // Update user profile with the selected image URI
        const updated = { ...loggedInUser, photoUrl: result.uri };
