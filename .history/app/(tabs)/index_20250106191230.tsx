...
// 1) Updated User interface to include 'name'
interface User {
  name: string;
  email: string;
  password: string;
...
const App: React.FC = (): React.ReactNode => {
  ...
  const [name, setName] = useState<string>(""); // NEW: For user’s name on signup
    ...
    const checkUserCredentials = async () => {
      try {
        ...
        if (storedUser) {
          ...
          // Auto-login to home if credentials exist
          setCurrentScreen("home");
          // Populate userProfile so the name shows on profile
          setUserProfile({
  ...
  // -----------------------------------------
  // Fetch YouTube video, events, ministries
  // -----------------------------------------
  useEffect(() => {
    fetchLatestVideo();
    fetchEvents();
  ...
  const fetchEvents = async () => {
    // Replace with your actual API or data
    const mockEvents = [
      {
        id: "1",
    ...
    const mockMinistries = [
      {
        id: "1",
        name: "Youth Ministry",
        description: "Engaging the youth through activities and programs."
      },
      {
        id: "2",
        name: "Music Ministry",
  ...
  // Prevent random single-letter login and ensure credentials exist
  ...
  // Sign up requires name, email, password, etc.
  const handleSignup = async () => {
    ...
    try {
      ...
      // Set userProfile so name shows up in Profile
      setUserProfile({
        name,
        bio: "Passionate about community service and outreach.",
        photoUrl: "https://example.com/user-photo.jpg",
      ...
      ListHeaderComponent={
        <>
          ...
          <View style={styles.profileIconsRow}>
            ...
            <TouchableOpacity style={styles.profileIconButton} onPress={() => {/* Navigate to Info */}}>
